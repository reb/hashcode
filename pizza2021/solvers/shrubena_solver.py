...
def best_pair(pizzas, pizza_compatibility_array, index_mask):
    ...
    max_idx = np.where(
        pizza_compatibility_array[np.ix_(index_mask, index_mask)]
        == np.max(pizza_compatibility_array[np.ix_(index_mask, index_mask)])
    )  # find entries which are maximal
...
def best_triple(pizza, pizza_compatibility_array, available_pizzas):
    ...
    two_pizzas_index = np.array(
        [available_pizzas[best_two_pizzas[0]], available_pizzas[best_two_pizzas[1]]]
    )
    ...
    fitness_column_pizza = pizza_compatibility_array[
        available_pizzas, best_two_pizzas[0]
    ]
    ...
    best_third_pizza_position = np.unravel_index(
        np.argmax(yummy_pizza, axis=None), yummy_pizza.shape
    )
    ...
    score = score_pizza(
        pizzas[three_pizzas[0]], pizzas[three_pizzas[1]], pizzas[three_pizzas[2]]
    )
